...
export default function ModelsPage() {
  const snapshot = useAgentStore(useShallow((s: AgentStoreSnapshot) => ({
  ...
  const searchHF = () => {
    ...
    setHfResults(q ? allRepos.filter((r) => r.id.toLowerCase().includes(q)) : allRepos);
  ...
  const runEval = async () => {
    ...
    await streamAgentEvents({
      url: "/api/eval/stream",
      init: { method: "POST", body: JSON.stringify({ runId: snapshot.trainingRunId, k: 3 }) },
  ...
  const tryModel = async (model: "base" | "tuned") => {
    ...
    if (model === "base") setBaseOutput(code); else setTunedOutput(code);
  };
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="mt-6 animate-reveal" style={{ animationDelay: "0.05s" }}>
        <div className="flex gap-2">
          <input className="h-10 flex-1 rounded-lg border border-white/10 bg-black px-3 text-sm text-white outline-none focus:border-emerald-300 placeholder:text-zinc-600 font-mono"
            placeholder="Filter models…" value={hfSearch}
            onChange={(e) => setHfSearch(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") searchHF(); }} />
          <Button onClick={searchHF} disabled={hfSearching}>
            {hfSearching ? <Loader2 className="size-4 animate-spin" /> : <Search className="size-4" />} Filter
          </Button>
        </div>
        {error && <p className="mt-2 text-xs text-amber-300">{error} — showing cached results</p>}
      </section>
      ...
      <section className="mt-4 animate-reveal" style={{ animationDelay: "0.1s" }}>
        {loading ? (
          <div className="flex flex-col items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.01] p-12 text-center">
            ...
            <p className="text-sm text-zinc-500">Loading models from Hugging Face…</p>
          </div>
        ) : hfResults.length > 0 ? (
            ...
            {hfResults.map((r) => (
              <button key={r.id} type="button"
                className={cn(
                  ...
                  selectedModel === r.id ? "bg-emerald-500/10 border border-emerald-500/20" : "text-zinc-300",
                )}
                onClick={() => setSelectedModel(r.id)}>
                <Package className="size-5 shrink-0 text-zinc-500" />
                <div className="min-w-0 flex-1">
                  <span className="block truncate font-mono text-white">{r.label}</span>
                  <span className="block text-xs text-zinc-500">LoRA adapter</span>
                </div>
                <a href={"https://huggingface.co/" + r.id} target="_blank" rel="noopener noreferrer"
                  className="shrink-0 text-zinc-600 hover:text-zinc-400" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex flex-col items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.01] p-12 text-center">
            ...
            <p className="max-w-sm text-sm leading-6 text-zinc-500">
              No models found. Push a LoRA adapter via the rehearsal script and it will appear here.
            </p>
          </div>
        )}
      </section>
