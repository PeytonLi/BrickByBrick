...
export interface NarrationBridgeOptions {
  ...
  connectGemini?: (callbacks: GeminiConnectorCallbacks) => Promise<GeminiAudioClient>
  createPublisher?: (options: LiveKitPublisherOptions) => Promise<LiveKitAudioPublisher>
}
...
class GeminiLiveNarrationBridge implements NarrationAudioBridge {
  ...
  private activeTurn: { resolve: () => void; reject: (error: unknown) => void } | null = null

  constructor(private readonly options: NarrationBridgeOptions) {
    const sampleRate =
      options.sampleRate ?? positiveInteger(process.env.GEMINI_LIVE_SAMPLE_RATE, DEFAULT_GEMINI_LIVE_SAMPLE_RATE)
  ...
  private async connect(sampleRate: number, channels: number): Promise<void> {
    ...
    const identity = this.options.identity ?? `narrator-${randomUUID().slice(0, 8)}`
    ...
    if (clientResult.status === 'rejected' || publisherResult.status === 'rejected') {
    ...
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        if (this.activeTurn) {
          ...
          reject(new Error('Gemini Live narration timed out before turnComplete'))
        }
      }, this.turnTimeoutMs)
...
function defaultGeminiConnector(modelOverride?: string) {
  return async function connectGeminiLive({
    ...
    const session = await ai.live.connect({
      model: modelOverride ?? process.env.GEMINI_LIVE_MODEL ?? DEFAULT_LIVE_MODEL,
      config: {
