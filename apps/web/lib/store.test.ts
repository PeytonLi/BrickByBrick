...
describe('reduceAgentState', () => {
  it('records every AgentEvent variant and keeps committed counters tied to pair_committed', () => {
    const events: AgentEvent[] = [
      ...
      { type: 'strong_fix_generated', code: pair.strong_code, diff: '+ min-w-0' },
      { type: 'audit_pass' },
      { type: 'pair_rejected', reason: 'too_easy' },
      { type: 'recipe_mutated', patch: { focus_mechanism: 'modal-focus-trap' } },
      { type: 'narration', text: 'Rejecting the easy case and mutating the recipe.' },
      {
        type: 'training_event',
        status: 'training',
    ...
    expect(state.narration).toContain('Rejecting the easy case and mutating the recipe.')
  ...
  it('updates audit screenshots on audit_step', () => {
    ...
    expect(state.latestScreenshotSrc).toBe(`data:image/png;base64,${screenshot}`)
