import type { AuditStep, DefectCategory, Severity, Viewport } from '@brickbybrick/core'
...
function outputTextFromSteps(steps: InteractionStep[]): string {
  ...
  for (const s of steps) {
    ...
    for (const c of s.content) if (typeof c.text === 'string') parts.push(c.text)
  }
  return parts.join('\n')
}
...
export function parseInteractionStream(raw: string): InteractionResult {
  const result: InteractionResult = { id: '', environmentId: '', status: '', steps: [], outputText: '' }
...
async function streamInteraction(body: unknown, opts: StreamOptions): Promise<InteractionResult> {
  return withRetry(async () => {
    ...
    if (!res.ok || !res.body) {
      throw new Error(`Antigravity interactions → ${res.status}: ${await res.text()}`)
    }
...
export async function createInteraction(prompt: string, opts: StreamOptions = {}): Promise<InteractionResult> {
...
function sentinelTexts(result: InteractionResult): string[] {
  const texts: string[] = []
  for (const s of result.steps) if (typeof s.result === 'string') texts.push(s.result)
...
export function parseAuditStepsFromText(text: string): AuditStep[] {
  ...
  while ((m = AUDIT_STEP_RE.exec(text))) {
    let obj: { action?: unknown; intent?: unknown; viewport?: unknown; thumbnail?: unknown }
    try {
      obj = JSON.parse(m[1].trim())
    } catch {
      continue
    }
    const thumbnail = typeof obj.thumbnail === 'string' ? stripDataUri(obj.thumbnail) : ''
    out.push({
...
export function frameDeltaText(frame: { delta?: unknown; [k: string]: unknown }): string {
  const delta = frame.delta as { result?: unknown; content?: { text?: unknown } } | undefined
  if (delta && typeof delta.result === 'string') return delta.result
  if (delta?.content && typeof delta.content.text === 'string') return delta.content.text
  return ''
}
...
export function parseAuditReport(result: InteractionResult): AuditReport {
  const text = [result.outputText, ...result.steps.map((s) => (typeof s.result === 'string' ? s.result : ''))].join('\n')
  ...
  if (m) {
    try {
      ...
      return {
        ...
        passedCriteria: Array.isArray(v.passed_criteria) ? (v.passed_criteria as string[]) : [],
        failedCriteria: Array.isArray(v.failed_criteria) ? (v.failed_criteria as string[]) : [],
  ...
  return { passed, passedCriteria: [], failedCriteria: [], domTrace: '', notes: text.slice(0, 4000) }
}

/** Download the sandbox file system (full-res PNGs, JSONL artifacts) as a TAR Buffer. */
export async function downloadEnvironment(environmentId: string, opts: RetryOptions = {}): Promise<Buffer> {
  return withRetry(async () => {
    const res = await fetch(`${GEMINI_BASE}/files/environment-${environmentId}:download?alt=media`, {
      headers: { 'x-goog-api-key': apiKey() },
    })
    if (!res.ok) {
      throw new Error(`Antigravity download ${environmentId} → ${res.status}: ${await res.text()}`)
    }
...
export async function destroyEnvironment(environmentId: string, opts: RetryOptions = {}): Promise<void> {
  await withRetry(async () => {
    const res = await fetch(`${GEMINI_BASE}/files/environment-${environmentId}`, {
      method: 'DELETE',
      headers: { 'x-goog-api-key': apiKey() },
    })
    if (!res.ok) {
      throw new Error(`Antigravity destroy ${environmentId} → ${res.status}: ${await res.text()}`)
    }
  }, opts)
}
