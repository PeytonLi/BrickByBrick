...
function applyPatch(config: GenerationConfig, patch: Partial<GenerationConfig>): GenerationConfig {
  return {
    ...config,
    ...patch,
...
export const runVisualLoop = async (
  ...
  const maxIterations = deps.maxIterations ?? Math.max(50, current.max_pairs * 20)
  let iterations = 0

  emit({ type: 'narration', text: `Starting visual loop — target ${current.max_pairs} pairs.` })

  while (committed.length < current.max_pairs && iterations < maxIterations) {
    iterations++

    // 1. Challenger → VisualTask
    emit({ type: 'narration', text: 'Challenger is designing an adversarial UI task…' })
    const task = await deps.challenge(current)
    emit({ type: 'challenge_generated', task })

    // 2. Weak / target model draft
    emit({ type: 'narration', text: 'Weak model is drafting an implementation…' })
    ...
    if (weakAudit.passed || !weakAudit.defect) {
      ...
      emit({ type: 'narration', text: 'Weak model passed — too easy, discarding.' })
      continue
    }
    ...
    if (consecutiveFails >= 3) {
      if (current.focus_mechanism !== task.target_mechanism) {
        const patch: Partial<GenerationConfig> = { focus_mechanism: task.target_mechanism }
    ...
    emit({ type: 'strong_fix_generated', code: strongCode, diff: makeDiff(weakCode, strongCode) })
    ...
    if (!strongAudit.passed) {
      emit({ type: 'narration', text: 'Strong fix did not pass re-audit — discarding.' })
      continue
    }
    ...
    if (redundant) {
      ...
      emit({ type: 'narration', text: 'Failure too similar to a recent one — rejecting as redundant.' })
      continue
    }
...
function buildChallengerPrompt(config: GenerationConfig): string {
  ...
  const weighted = Object.entries(config.challenger_weights)
    .filter(([, w]) => w > 1)
    .map(([m]) => m)
  if (weighted.length) parts.push(`Prefer these mechanisms: ${weighted.join(', ')}.`)
  return parts.join('\n\n')
}
 ...
 */
function toAuditResult(task: VisualTask, steps: AuditStep[], report: ReturnType<typeof parseAuditReport>): AuditResult {
  if (report.passed) {
    return { passed: true, passedCriteria: task.criteria.map((c) => c.id), steps }
  }
  const defect: Defect = {
  ...
  return { passed: false, passedCriteria: report.passedCriteria, defect, steps }
}
...
export function defaultDeps(): VisualLoopDeps {
  return {
    ...
    weakSolver,
    strongSolver: (task, defect, weakCode) => strongSolver(task, defect, weakCode),
    audit: async (task, code, emit) => {
      ...
      const emitNew = (steps: AuditStep[]) => {
        for (; emitted < steps.length; emitted++) emit({ type: 'audit_step', step: steps[emitted] })
      }
