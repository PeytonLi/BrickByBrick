/**
 * Gemini client for the engine: the strong solver (Gemini 3.5 Pro), the weak /
...
export const STRONG_MODEL = () => process.env.STRONG_MODEL || 'gemini-3.1-pro-preview'
...
export async function weakSolver(task: VisualTask, opts: RetryOptions = {}): Promise<string> {
  const out = await generateContent(WEAK_MODEL(), WEAK_SOLVER_SYSTEM, describeTask(task), opts)
  return stripCodeFences(out)
}

/**
 * The strong model's fix for the audited defect (Gemini 3.5 Pro). Follows the
...
export async function strongSolver(
  ...
  parts.push('Return the corrected implementation that passes every criterion.')
  const out = await generateContent(STRONG_MODEL(), STRONG_SOLVER_SYSTEM, parts.join('\n\n'), opts)
  return stripCodeFences(out)
}

/** Embed text into a vector for the cosine diversity gate (MATH §3). */
export async function embed(text: string, opts: RetryOptions = {}): Promise<number[]> {
  return withRetry(async () => {
    ...
    if (!res.ok) {
      throw new Error(`Gemini embed ${model} → ${res.status}: ${await res.text()}`)
    }
