...
describe('buildTrainingConfig', () => {
  it('emits valid TOML with default params', () => {
    ...
    expect(toml).toContain('base = "google/gemma-4-9b-it"')
    ...
    expect(toml).toContain('lr = 0.0002')
  })

  it('includes lora_target_modules as TOML array', () => {
    const toml = buildTrainingConfig()
    expect(toml).toContain('target_modules = ["q_proj", "v_proj", "k_proj", "o_proj"]')
  })
  ...
  it('overrides lr', () => {
    ...
    expect(toml).not.toContain('lr = 0.0002')
  })
