...
const pair: TrainingPair = {
  ...
  task: {
    ...
    criteria: [{ id: 'no-overflow', description: 'No horizontal overflow at 375px', weight: 1 }],
  },
  weak_code: '<div className="grid grid-cols-3 gap-4"><article className="w-96">Long content</article></div>',
  defect: {
    screenshot: '',
...
async function main() {
  if (process.env.BBB_ALLOW_PAID_REHEARSAL !== '1') {
    console.error('Set BBB_ALLOW_PAID_REHEARSAL=1 to launch paid Prime H100 compute.')
    process.exit(1)
  }
  ...
  const result = await runGemmaLoraTraining(
    {
      pairs: [pair],
      runName: `bbb-rehearsal-${Date.now()}`,
      maxSteps: Number(process.env.BBB_TRAINING_MAX_STEPS ?? 5),
      keepPod: process.env.BBB_KEEP_POD === '1',
    },
    {
      onStatus: (status, detail) => console.log(JSON.stringify({ type: 'status', status, detail })),
      onMetric: (loss) => console.log(JSON.stringify({ type: 'metric', ...loss })),
      onLog: (line) => {
